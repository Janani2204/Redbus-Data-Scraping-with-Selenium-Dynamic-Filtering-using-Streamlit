...
def get_connection():
    return pymysql.connect(host='localhost', user='root', passwd='root', database='redbus_data')
...
def fetch_data(connection, route_name, price_sort_order):
    ...
    query = f"SELECT * FROM redbus WHERE Route_Name = %s ORDER BY Star_Rating DESC, Price {price_sort_order_sql}"
